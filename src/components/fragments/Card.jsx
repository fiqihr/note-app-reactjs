...
const Card = (props) => {
  ...
  const handleUpdate = (newTitle, newContent) => {
    setCardData({ title: newTitle, content: newContent });
  };
  ...
  return (
    <>
      ...
      >
        ...
        {props.date ? showFormattedDate(props.date) : "Tanggal tidak tersedia"}
        </p>
        <hr className="my-2" />
        <div className="h-20">
          <p className="line-clamp-3">{cardData.content || "Konten kosong"}</p>
        </div>
